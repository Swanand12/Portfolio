  ...
  useEffect(() => {
    ...
    function runMatter() {
      ...
      for (var i = 0; i < 60; i += 1) {
        ...
        let s =
          Common.random() > 0.6 ? Common.random(10, 80) : Common.random(10, 60);
      ...
      Events.on(engine, "afterUpdate", function () {
        ...
        Composite.allBodies(world).forEach((body) => {
          if (body === attractiveBody || body.isStatic) return; // Skip attractor and static bodies

          const deltaX = attractiveBody.position.x - body.position.x;
          const deltaY = attractiveBody.position.y - body.position.y;
          const distanceSq = deltaX * deltaX + deltaY * deltaY;

          if (distanceSq < 100) return; // Avoid applying force if bodies are very close

          const forceMagnitude = (0.1 * body.mass) / distanceSq;
